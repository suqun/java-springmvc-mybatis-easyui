package snipe.interceptors.pagination;
...
/**
 * 
 * pagination : PageInterceptor.java
 * 
 * @date 2013-9-29
 * @author zhu8fei Wong
 * @version
...
public class PageInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (obj instanceof PageHelper) {
			PageHelper page = (PageHelper) obj;
			if (!page.isQueryAll()) {
	 ...
	 * @return
	 */
	private String getPageSql(PageHelper page, String sql) {
...
//		sqlBuffer.insert(0, "SELECT VU.*, ROWNUM R FROM (").append(
//				") VU WHERE ROWNUM < ").append(offset + page.getPageSize());
//		sqlBuffer.insert(0, "SELECT * FROM (").append(") WHERE R >= ").append(
//				offset);
//		return sqlBuffer.toString();
//	}
	 ...
	 *            当前的数据库连接
	 */
	private void setTotalRecord(PageHelper page, MappedStatement mappedStatement,
		...
		try {
			...
			if (rs.next()) {
				Long totalRecord = (long) rs.getInt(1);
				// 给当前的参数page对象设置总记录数
